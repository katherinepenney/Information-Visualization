...
tweets <- read.csv("C:/Users/Katherine Penney/Desktop/climatetweets_useforlecture_25k.csv", header = TRUE,stringsAsFactors = FALSE ,quote = "\"")
View(tweets)
...
wordcloud(df$words[index], my.counts, scale = c(4, .4), 
          ...
          random.color = FALSE, ordered.colors = TRUE, rot.per=0, myPal(length(df$words[index])))

####


sales<- read.csv("C:/Users/Katherine Penney/Desktop/sales (1).csv", header = TRUE, stringsAsFactors = FALSE)
library(alluvial)
